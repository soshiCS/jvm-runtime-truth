...
3. **The generated class name (`Transform037`) IS visible** in the Undo
   ...
   path — essentially the same as the Runtime Truth path, achieved via different means.
...
5. **INSTANCE_TOKEN is not a barrier for Undo**. In a recorded execution, the
   ...
   blocks Agent A (in the Runtime Truth benchmark) does not block a time-travel
   debugger.
...
|---|---|---|---|
| **Reflection target attribution** | ✓ Automatic — REST API returns callsite→target | ✓ Via recording — navigate to invocation, inspect frame | Both see it; Runtime Truth is structured/queryable |
| **Polymorphic dispatch attribution** | ✓ `callsite_target` records per invocation | ✓ Step to invocation, inspect type | Both see it |
| **Generated class identification** | ✓ Class name in `generated_class` record; `artifact_javap` API | ✓ Class name visible in debugger at invocation | Both see it; Runtime Truth delivers it as data |
| **Hidden class visibility** | ✓ Explicit `hidden_class_identity` records with CRC | ✓ Hidden classes appear in debugger call stack | Both see it; Runtime Truth attributes provenance (LambdaMetafactory, invokedynamic) |
| **Runtime target attribution** | ✓ Per-callsite attribution across full execution | ✓ Same, via replay | Both; Runtime Truth is queryable without replay |
| **Generated bytecode inspection** | ✓ `artifact_javap` REST API — baked constants visible | ✓ Debugger decompiler shows class; stepping reveals values | Both; Runtime Truth is API-accessible |
| **Artifact retrieval** | ✓ `artifact_path` in JSONL — `.class` file on disk | ✗ No explicit artifact export API | Runtime Truth saves artifacts explicitly |
| **Loader attribution** | ✓ `loader_id` per class in JSONL | Partial — visible in debugger, not as structured data | Runtime Truth is more explicit |
| **Causality chain reconstruction** | ✓ `causality_chain` REST endpoint | Manual — navigate recording step by step | Runtime Truth is automatic; Undo requires interactive navigation |
| **AI-agent friendliness** | ✓ Structured REST APIs; designed for programmatic access | ✗ Interactive debugger; not API-accessible | **Runtime Truth's primary differentiator** |
...
| **Cross-request correlation** | ✓ JSONL annotated per-callsite across full process run | Partial — one recording per session | Runtime Truth advantage for multi-request analysis |
| **macOS support** | ✓ | ✗ Recording requires Linux | Runtime Truth advantage for dev environments |
| **Structured data output** | ✓ JSONL + REST | ✗ Interactive only | Runtime Truth advantage for tooling integration |
| **Recording overhead** | None — JVM instrumentation at load time | Significant — records every instruction | Runtime Truth advantage for production use |
...
| **Full memory history** | At any point in the recording, Undo can show the exact heap state. Runtime Truth records class/method attribution, not data values. |
| **Variable value inspection** | Can see the value of `keyValue` at the moment `FixedValue.value(keyValue)` was called. Runtime Truth only sees that the call happened. |
| **Deterministic replay** | The same bug can be replayed identically for collaborative investigation. Runtime Truth's live capture is not replayable. |
...
| **Zero recording overhead in production** | Runtime Truth instruments at class load time. There is no per-execution recording cost. Undo's recording imposes significant overhead. |
| **macOS recording** | Works on macOS. Undo cannot record on macOS. |
| **No replay required** | Runtime Truth captures all data during normal execution. No special reproduction step is needed to get attribution data. |
...
| Advantage vs. Undo on non-generated-class bugs | Unproven — V5 was designed to favor Runtime Truth |
